...
| `server` | Accept loop, per-client handlers, online registry, in-memory history     |
...
- **Accept loop** — `ChatServer.start()` blocks on `accept()` and hands each socket to a
  cached thread pool.
- **One thread per client** — `ClientHandler` owns the socket and reads until the client
  quits or disconnects.
...
The name exchange predates the JSON protocol and is still half plain text:

1. Server to client: `SYSTEM` message with body `Enter your name:`
2. Client to server: **a raw line of text** with the desired name (not JSON)
3. If the name is taken, the server replies `ERROR` and prompts again; otherwise it replays
   history and announces the join to everyone else.
...
| `LOGIN`     | authenticate                      | reserved    |
| `REGISTER`  | create an account                 | reserved    |
...
| `LOGIN_SUCCESS`    | authentication accepted                        | reserved    |
| `LOGIN_FAILURE`    | authentication rejected                        | reserved    |
...
- `users` — `id`, `username` (unique), `password_hash`, `created_at`. Written by
  `UserRepository`; not used by the chat flow until authentication lands.
...
- [ ] **Auth** — `LOGIN` / `REGISTER` against the `users` table, password hashing, and
      `AuthenticationException` / `UsernameTakenException` wired into the handshake, so a
      name is owned by an account rather than claimed first-come.
